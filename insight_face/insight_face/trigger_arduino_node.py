...
class TriggerLedNode(Node):
    def __init__(self):
        super().__init__('trigger_arduino_node')

        self.declare_parameter('arduino_port', '/dev/ttyUSB0')
        self.declare_parameter('arduino_baud_rate', 9600)
        self.arduino_port = self.get_parameter('arduino_port').get_parameter_value().string_value
        self.arduino_baud_rate = self.get_parameter('arduino_baud_rate').get_parameter_value().integer_value
        ...
        self.led_blink_status = 0  # 0: not in use, 1: in use
    ...
    def setup_serial_connection(self):
        try:
            self.arduino = serial.Serial(self.arduino_port, self.arduino_baud_rate, timeout=1)
            self.get_logger().info(f"Connected to Arduino on {self.arduino_port} at {self.arduino_baud_rate} baud.")
    ...
    def listener_callback(self, msg):
        ...
        if recognized_name in self.reference_list and recognized_name not in self.updated_recognized_list:
            if self.led_blink_status:
                self.get_logger().info("LED is currently being used. Found another person in detection, skipping LED action until 10 seconds complete.")
            else:
    ...
    def flash_leds(self):
        """Send a command to Arduino to flash LEDs for 10 seconds."""
        self.get_logger().info("Match found! Sending signal to Arduino to flash LEDs for 10 seconds.")
        try:
            ...
            self.get_logger().error(f"Failed to send signal to Arduino: {e}")
    ...
    def gpio_callback(self):
        ...
        if self.led_counter >= 10:
            ...
            self.get_logger().info(f'Blinking LED for remaining {10 - self.led_counter} seconds.')

    def destroy_node(self):
        """Clean up before shutting down."""
        try:
            self.arduino.close()
            self.get_logger().info("Closed Arduino connection.")
        except serial.SerialException as e:
            self.get_logger().error(f"Failed to close Arduino connection: {e}")
        super().destroy_node()
